...
public class Investor extends Player {
	...
	public Investor(int typ) {
		...
		this.expertise = RandomHelper.createUniform(CONSUMER_TYPE_L, CONSUMER_TYPE_H).nextInt();
		Parameters params = RunEnvironment.getInstance().getParameters(); // pobranie
																			// parametr—w
	...
	private HYIPowner chooseProducer() {
		...
		return (HYIPowner) context.getRandomObjects(HYIPowner.class, 1)
				.iterator().next();
	}
	...
	/*
	 * public static void evolve(){ IndexedIterable<Player> allConsumers =
	 * CredibilityGame.PLAYERS.getObjects(Investor.class);
	 * HashMap<Double,ArrayList<Player>> consumers = new HashMap<Double,
	 * ArrayList<Player>>(); for(double type:EXPERTISE_LEVELS){
	 * consumers.put(type, new ArrayList<Player>()); } for(Object
	 * consumer:allConsumers){
	 * consumers.get(((Investor)consumer).getExpertise())
	 * .add(((Investor)consumer)); } for(double type:consumers.keySet()){
	 * Player.stochasticSampling(consumers.get(type)); } Iterable<Player>
	 * mutatedConsumers =
	 * CredibilityGame.PLAYERS.getRandomObjects(Investor.class,
	 * (int)(allConsumers.size()*0.01)); for(Object c:mutatedConsumers){
	 * ((AcceptanceStrategy
	 * )((Investor)c).getStrategy()).setThreshold(random.nextDouble()); } }
	 */
