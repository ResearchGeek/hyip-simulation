...
public class HYIPowner extends Player{
	...
	public static void evolve(){
		IndexedIterable<Player> allProducers = CredibilityGame.PLAYERS.getObjects(HYIPowner.class);
		...
		//	if(((HYIPowner)producer).isHonest())
		//		producersListHonest.add(((HYIPowner)producer));
		//	else
		...
		Iterable<Player> mutatedProducers = CredibilityGame.PLAYERS.getRandomObjects(HYIPowner.class, (int)(allProducers.size()*0.01));
		for(Object p:mutatedProducers){
			((ProducerStrategy)((HYIPowner)p).getStrategy()).setLook(random.nextInt(2));
		}
	}
	*/
