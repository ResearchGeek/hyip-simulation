...
public class AcceptanceStrategy implements Strategy {
	...
	public static void initialize() {
		...
		// maxMutation =
		// (Double)params.getValue("consumer_max_threshold_mutation");
		// signalTruthfulnessWeight =
		// (Double)params.getValue("signal_truthfulness_weight");
	...
	public AcceptanceStrategy(Investor consumer) {
		...
		reputationThreshold = RandomHelper.createUniform(0,
				((Integer) params.getValue("consumer_population_size") / 2))
				.nextInt();
		this.consumer = consumer;
		acceptanceStrategy = (Integer) params
	...
	private AcceptanceStrategy(double threshold, double reputationThreshold,
	...
	private boolean isAcceptableByReputation(Information i) {
		Context<Object> parentContext = ContextUtils
				.getParentContext(ContextUtils.getContext(consumer));
		GameController controller = (GameController) parentContext.getObjects(
				GameController.class).get(0);
		return controller.isWarmedUp() ? i.getStrategy().getProducer()
				.getCurrentRating().getRatingAsDouble() > reputationThreshold
				: true;
	}

	private void accept(Information i, boolean acceptedBySignal,
			boolean acceptedByReputation) {
		i.accept(consumer, acceptedBySignal, acceptedByReputation);
	}

	private void reject(Information i, boolean acceptedBySignal,
			boolean acceptedByReputation) {
	...
	public Strategy copy() {
		return new AcceptanceStrategy(getThreshold(), getReputationThreshold(),
	...
	public void copyStrategy(Strategy copyFrom) {
		...
		double mutation = (random.nextDouble() * (maxMutation * 2))
				- maxMutation;
		double newThreshold = (((AcceptanceStrategy) copyFrom).getThreshold())
				+ mutation;
		if (newThreshold < 0)
			newThreshold = 0;
		...
		newThreshold = (((AcceptanceStrategy) copyFrom)
				.getReputationThreshold()) + mutation;
		// newThreshold = 1000;
		if (newThreshold < 0)
			newThreshold = 0;
		setReputationThreshold(newThreshold);
		setAcceptanceStrategy(((AcceptanceStrategy) copyFrom)
				.getAcceptanceStrategy());
	}
}
